...
function readinpmline (data, lr) {
  data.inpm = lr.readInt('inpm=', 1, true)[0]
  data.iphas = lr.readInt('iphas=', 1)[0]
}

function rdlc (data, lr) {
  data.isys = lr.readInt('isys=', 1, true)[0]
  rdac(data, lr)
  rdeps(data, lr)
  data.ists = lr.readInt('ists=', 1)[0]
  // setlcnst(data)
  // initalamcl(data)
}

function rdac (data, lr) {
  data.al = lr.readReal('a=', 1)[0]
  data.am = data.al
  if (data.isys <= 2) { // icosahedral
    data.ndm = 6
  } else { // 2-dim qc
...
/*
function setlcnst (data) {
  if (data.isys <= 2) {
    const al = data.al
    data.lcnst = [al, al, al, al, al, al]
  } else {
    const al = data.al
    const cl = data.cl
    data.lcnst = [al, al, cl, al, al, 0]
  }
}

function initalamcl (data) {
  if (data.isys <= 2) {
    const al = data.al
    const am = data.am
    const ali = 1 / al
    const ami = 1 / am
    data.dalamcl = [al, al, al, am, am, am]
    data.rdalamcl = [ali, ali, ali, ami, ami, ami]
  } else {
    const al = data.al
    const cl = data.cl
    const am = data.am
    ...
    const cli = 1 / cl
    const ami = 1 / am
    data.dalamcl = [al, al, cl, am, am, 1]
    data.rdalamcl = [ali, ali, cli, ami, ami, 1]
  }
}
*/

function readnaline (data, lr) {
  data.na = lr.readInt('na=', 1, true)[0]
  data.npod = lr.readInt('npod=', 1)[0]
  data.ne = lr.readInt('ne=', 1)[0]
  data.isim = lr.readInt('isim=', 1)[0]
  data.imod = lr.readInt('imod=', 1)[0]
}
...
function setd (data) {
  switch (data.isys) {
    case 1:
      {
        ...
        const ph = 2 * Math.PI / 5
        const c1 = Math.cos(ph)
        const c2 = Math.cos(ph * 2)
        const c3 = Math.cos(ph * 3)
        const c4 = Math.cos(ph * 4)
        const c5 = 1
        const s1 = Math.sin(ph)
        const s2 = Math.sin(ph * 2)
        const s3 = Math.sin(ph * 3)
        const s4 = Math.sin(ph * 4)
        const s5 = 0
        data.dr = [
          0, c2 * st, c3 * st, c4 * st, c5 * st, c1 * st,
          0, s2 * st, s3 * st, s4 * st, s5 * st, s1 * st,
          1, ct, ct, ct, ct, ct,
          0, -c4 * st, -c1 * st, -c3 * st, -c5 * st, -c2 * st,
          0, -s4 * st, -s1 * st, -s3 * st, -s5 * st, -s2 * st,
          1, -ct, -ct, -ct, -ct, -ct]
      }
      break
    case 2:
    ...
    case 3:
      {
        ...
        const s4 = Math.sin(ph * 4)
        data.dr = [
          (c1 - 1) * f, (c2 - 1) * f, (c3 - 1) * f, (c4 - 1) * f, 0,
          s1 * f, s2 * f, s3 * f, s4 * f, 0,
          0, 0, 0, 0, 1,
          (c2 - 1) * f, (c4 - 1) * f, (c1 - 1) * f, (c3 - 1) * f, 0,
          s2 * f, s4 * f, s1 * f, s3 * f, 0]
      }
      break
    case 4:
      {
        const ph = 2 * Math.PI / 8
        const c0 = 1
        const c1 = Math.cos(ph)
        const c2 = Math.cos(ph * 2)
        const c3 = Math.cos(ph * 3)
        const c6 = Math.cos(ph * 6)
        const c9 = Math.cos(ph * 9)
        const s0 = 0
        const s1 = Math.sin(ph)
        const s2 = Math.sin(ph * 2)
        const s3 = Math.sin(ph * 3)
        const s6 = Math.sin(ph * 6)
        const s9 = Math.sin(ph * 9)
        data.dr = [
          c0, c1, c2, c3, 0,
          s0, s1, s2, s3, 0,
          0, 0, 0, 0, 1,
    ...
    case 5:
      {
        const ph = 2 * Math.PI / 12
        const cm1 = Math.cos(-ph)
        const c0 = 1
        const c1 = Math.cos(ph)
        const c3 = Math.cos(ph * 3)
        const c4 = Math.cos(ph * 4)
        const c6 = Math.cos(ph * 6)
        const c21 = Math.cos(ph * 21)
        const c26 = Math.cos(ph * 26)
        const sm1 = Math.sin(-ph)
        const s0 = 0
        const s1 = Math.sin(ph)
        const s3 = Math.sin(ph * 3)
        const s4 = Math.sin(ph * 4)
        const s6 = Math.sin(ph * 6)
        const s21 = Math.sin(ph * 21)
        const s26 = Math.sin(ph * 26)
        const f = Math.sqrt(2 / 3)
        data.dr = [
          cm1 * f, c0 * f, c3 * f, c4 * f, 0,
          sm1 * f, s0 * f, s3 * f, s4 * f, 0,
          0, 0, 0, 0, 1,
          c1 * f, c6 * f, c21 * f, c26 * f, 0,
          s1 * f, s6 * f, s21 * f, s26 * f, 0]
      }
      break
    default:
      throw Error(`
